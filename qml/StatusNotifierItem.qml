...
AbstractItem {
    ...
    itemId: model?.Id ?? ""
    text: presentationText || model?.Title || model?.ToolTipTitle || ""
    ...
    inlineIcon: {
        ...
        if (model?.status === PlasmaCore.Types.NeedsAttentionStatus) {
            return model?.AttentionIcon || model?.AttentionIconName || model?.Icon || model?.IconName || "";
        }
