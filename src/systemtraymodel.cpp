...
QHash<int, QByteArray> BaseModel::roleNames() const
{
    return {
        ...
        {static_cast<int>(BaseRole::ControlCenterOrder), QByteArrayLiteral("controlCenterOrder")},
    };
}
...
void BaseModel::onConfigurationChanged()
{
    ...
    Q_EMIT dataChanged(index(0, 0), index(rowCount() - 1, 0), {
        static_cast<int>(BaseModel::BaseRole::EffectiveStatus),
        static_cast<int>(BaseModel::BaseRole::ControlCenterOrder),
    });
}
...
int BaseModel::controlCenterOrder(const QString &itemId) const
{
    const int configuredIndex = m_settings
        ? m_settings->controlCenterItems().indexOf(itemId) : -1;
    return configuredIndex >= 0 ? configuredIndex : 100;
}
...
QVariant PlasmoidModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role < static_cast<int>(Role::Applet)) {
        ...
        case BaseRole::ControlCenterOrder: {
            return controlCenterOrder(pluginMetaData.pluginId());
        }
        default:
            return {};
        }
